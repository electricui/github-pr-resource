	...
	for _, tc := range tests {
		t.Run(tc.description, func(t *testing.T) {
			...
			if assert.Equal(t, 1, git.PullCallCount()) {
				url, base, depth, submodules, fetchTags, allowUnrelatedHistories := git.PullArgsForCall(0)
				...
				assert.Equal(t, tc.parameters.AllowUnrelatedHistories, allowUnrelatedHistories)
			}
